...
- **It does not validate responses at runtime.** The document says what an
  ...
  > **This happened.** One storefront listing paginated at runtime while the
  > document called it a plain array, because Scramble cannot follow a scope
  > reached through a relation. `make api-check` passed throughout - the
  > committed document was exactly what the code generated, and what the code
  > generated was wrong. [ADR 0022](0022-what-a-page-looks-like.md) records the
  > cause and the test that now asserts the document agrees.
